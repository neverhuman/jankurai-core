...
| `agent/vibe-coverage.toml` + `agent/vibe-source-inventory.toml` | `schemas/vibe-coverage-source.schema.json`; `vibe_coverage_smoke`, `vibe_coverage_semantic`, `schema_contracts` | Canonical source-row mapping plus the split core's complete row inventory, with reviewed canonical groups, detector status, and evidence status |
